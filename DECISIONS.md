...
**What's not built:** opening a document in place (needs a FileProvider/`open_filex`-style native viewer hookup — a second new native dependency) and "uploaded to the user's storage bucket on sync" (no sync backend exists yet, same blocker as everywhere else this session that needs one). The file's real, safely stored, and its metadata is genuinely useful even without in-app preview.
