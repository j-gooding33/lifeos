...
## §5.5 — `dashboardCardsProvider` could crash on the very first frame it ever ran (2026-08-29)
...
**Fix:** capture the repository once, before any `await` (it's `keepAlive` and synchronous, so there's no reason to re-fetch it), and check `ref.mounted` after each subsequent `await`, returning early rather than calling `ref.watch` again if the provider was torn down mid-flight — exactly what the error message itself recommends.
**Verified by reproducing the exact original sequence:** cleared app data again, walked through onboarding to Finish a second time, and confirmed zero `Uncaught`/`I/flutter` lines in logcat this time, versus a reliable repro before the fix.
