...
/// M8 Part 7: "My Top 5 Films" — manually curated and capped (Part 42:
/// independent of star ratings, nothing here is derived from a rating).
class FilmTop5Screen extends ConsumerWidget {
  const FilmTop5Screen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final colors = context.colors;
    const type = MediaType.film;
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('Top 5 films')),
      body: entriesAsync.when(
        loading: () => const Center(child: CircularProgressIndicator()),
        error: (error, stack) => LErrorState(message: "Couldn't load your Top 5.", onRetry: () => ref.invalidate(topListProvider(type))),
        data: (entries) {
          return itemsAsync.when(
            loading: () => const Center(child: CircularProgressIndicator()),
            error: (error, stack) => LErrorState(message: "Couldn't load your films.", onRetry: () => ref.invalidate(libraryAllProvider(type))),
            data: (allItems) {
              ...
              return Column(
                children: [
                  Expanded(
                    child: ranked.isEmpty
                        ? const LEmptyState(
                            ...
                            title: 'No Top 5 yet',
                            message: 'Add your favourite films below. A high star rating never adds one automatically.',
                          )
                        : ReorderableListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return LListTile(
                                ...
                                subtitle: row.item.year?.toString(),
                                trailing: IconButton(
                                  icon: const Icon(Icons.close),
                                  tooltip: 'Remove from Top 5',
                                  onPressed: () async {
                                    final userId = await ref.read(currentUserIdProvider.future);
                                    await repository.remove(userId, type, row.item.id);
                                  },
                                ),
                                onTap: () => context.push(Routes.libraryFilmDetail.replaceFirst(':id', row.item.id)),
                              );
                            },
                          ),
                  ),
                  Padding(
                    ...
                    child: LButton(
                      label: ranked.length >= repository.capFor(type) ? 'Top 5 is full' : 'Add a film',
  ...
  Future<void> _reorder(BuildContext context, WidgetRef ref, MediaType type, List<String> orderedIds) async {
    final userId = await ref.read(currentUserIdProvider.future);
        ...
        if (candidates.isEmpty) {
          return const Padding(
            ...
            child: LEmptyState(
              ...
              message: 'Add a film to your library first.',
            ),
          );
        }
