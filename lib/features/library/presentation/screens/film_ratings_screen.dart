...
/// M8 Part 5: rating history, sortable newest/oldest/highest/lowest — the
/// date used for newest/oldest is the watched date, falling back to when it
/// was added if it was rated before being watched (§43).
class FilmRatingsScreen extends ConsumerStatefulWidget {
  const FilmRatingsScreen({super.key});

  @override
  ConsumerState<FilmRatingsScreen> createState() => _FilmRatingsScreenState();
}

class _FilmRatingsScreenState extends ConsumerState<FilmRatingsScreen> {
  var _sort = _RatingSort.newest;

  @override
  Widget build(BuildContext context) {
    final colors = context.colors;
    final provider = libraryRatedProvider(MediaType.film);
    final asyncItems = ref.watch(provider);
    return Scaffold(
      backgroundColor: colors.neutrals.bg,
      appBar: AppBar(title: const Text('Film ratings')),
      body: Column(
        children: [
          Padding(
              ...
              data: (items) {
                if (items.isEmpty) {
                  return const LEmptyState(
                    ...
                    title: 'No rated films yet',
                    message: 'Rate a film from its detail page to see it here.',
                  );
                }
                ...
                return ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return LListTile(
                      leading: LPosterTile(width: 44, imageProvider: posterImageFor(ref, MediaType.film, item.posterPath)),
                      title: item.title,
