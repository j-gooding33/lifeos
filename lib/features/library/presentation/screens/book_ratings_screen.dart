...
/// Mirrors `film_ratings_screen.dart` — 1-5★, sortable, date falls back to
/// when the book was added if it was rated before being finished (§43).
class BookRatingsScreen extends ConsumerStatefulWidget {
  const BookRatingsScreen({super.key});

  @override
  ConsumerState<BookRatingsScreen> createState() => _BookRatingsScreenState();
}

class _BookRatingsScreenState extends ConsumerState<BookRatingsScreen> {
  var _sort = _RatingSort.newest;

  @override
  Widget build(BuildContext context) {
    final colors = context.colors;
    final provider = libraryRatedProvider(MediaType.book);
    final asyncItems = ref.watch(provider);
    return Scaffold(
      backgroundColor: colors.neutrals.bg,
      appBar: AppBar(title: const Text('Book ratings')),
      body: Column(
        children: [
          Padding(
              ...
              data: (items) {
                if (items.isEmpty) {
                  return const LEmptyState(
                    ...
                    title: 'No rated books yet',
                    message: 'Rate a book from its detail page to see it here.',
                  );
                }
                ...
                return ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return LListTile(
                      leading: LPosterTile(width: 44, imageProvider: posterImageFor(ref, MediaType.book, item.posterPath)),
                      title: item.title,
