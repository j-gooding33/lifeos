...
/// "My Top 5 TV Shows" — mirrors `film_top5_screen.dart` exactly (same
/// repository, same cap of 5, same independence from star ratings).
class TvTop5Screen extends ConsumerWidget {
  const TvTop5Screen({super.key});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('Top 5 TV shows')),
      body: entriesAsync.when(
        ...
        data: (entries) {
          return itemsAsync.when(
            loading: () => const Center(child: CircularProgressIndicator()),
            error: (error, stack) => LErrorState(message: "Couldn't load your shows.", onRetry: () => ref.invalidate(libraryAllProvider(type))),
            data: (allItems) {
              ...
              return Column(
                children: [
                  Expanded(
                    child: ranked.isEmpty
                        ? const LEmptyState(
                            ...
                            title: 'No Top 5 yet',
                            message: 'Add your favourite shows below. A high star rating never adds one automatically.',
                          )
                        : ReorderableListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return LListTile(
                                ...
                                onTap: () => context.push(Routes.libraryTvDetail.replaceFirst(':id', row.item.id)),
                              );
                            },
                          ),
                  ),
                  Padding(
                    ...
                    child: LButton(
                      label: ranked.length >= repository.capFor(type) ? 'Top 5 is full' : 'Add a show',
        ...
        if (candidates.isEmpty) {
          return const Padding(
            ...
            child: LEmptyState(
              ...
              message: 'Add a show to your library first.',
            ),
          );
        }
