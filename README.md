...
| Settings (§22.5) | Profile, Appearance, Home dashboard, Calendar and Privacy (real saved preferences, no delivery/collection behind them yet), Data (storage used, rebuild search index, clear image cache, export/import — see below), Integrations (honest TMDB/Open Library status), About (version, licences). Account and Subscription stay the honest "not built yet" placeholder — each needs a real auth session or IAP. |
| Data export & import (§27.5) | Export: every table (bar internal sync bookkeeping, re-fetchable metadata caches, and file-backed attachments) dumped to one JSON file via the OS share sheet. Import: accepts that same file, shows a dry-run row-count preview before writing anything, and is always additive (`INSERT OR IGNORE`) — never overwrites or duplicates existing data. The spec's ZIP bundle (a CSV spreadsheet export, CSV import mapping for other apps, attachments) isn't built; this is JSON-only. |
| Calendar (§14) | Unified Day/Month views merging tasks (due times), events and plan/habit occurrences from one range query per period. Device calendar import (§14.4, Android only) — opt-in, read-only: a master switch doubles as the OS permission request, then per-calendar toggles; imported events render in grey with a read-only detail view and are cleared the moment their calendar is turned off. Week and 3-day views, pinch-to-morph, and two-way sync aren't built. |
...
| Home-screen widget (§22.4) | Android only — "Today's tasks," placeable from the OS widget picker, showing up to 5 of today's incomplete tasks (or "Nothing today"), tap to open the app. Refreshes on app resume via a JSON data bridge (`home_widget`); native `RemoteViews`, no database or network access on the widget side. The other 6 widgets in the spec's table aren't built; iOS isn't built at all (no Xcode target exists in this environment). |
