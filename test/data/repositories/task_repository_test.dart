...
void main() {
  ...
  test('createTask then watchToday includes it when due today or earlier', () async {
    final today = CivilDate.fromDateTime(DateTime.now());
    await repository.createTask(userId: 'u1', title: 'Email Dr Hall', dueDate: today.toIso());

    final tasks = await repository.watchToday('u1', today).first;
    expect(tasks.map((t) => t.title), contains('Email Dr Hall'));
  });

  test('a task due tomorrow does not appear in today, but does in upcoming', () async {
    final today = CivilDate.fromDateTime(DateTime.now());
    final tomorrow = today.addDays(1);
    await repository.createTask(userId: 'u1', title: 'Future thing', dueDate: tomorrow.toIso());
  ...
  test('completing a task sets completedAt and it leaves the open views', () async {
    final today = CivilDate.fromDateTime(DateTime.now());
    final created = await repository.createTask(userId: 'u1', title: 'Buy milk', dueDate: today.toIso());
    final task = created.when(ok: (t) => t, err: (_) => throw StateError('expected Ok'));
  ...
  test('completing a repeating task creates the next instance immediately (§10.6)', () async {
    final today = CivilDate.fromDateTime(DateTime.now());
    ...
    final next = result.when(ok: (t) => t, err: (_) => throw StateError('expected Ok'));
  ...
  test('a non-repeating task produces no next instance on completion', () async {
    final today = CivilDate.fromDateTime(DateTime.now());
    final created = await repository.createTask(userId: 'u1', title: 'One-off', dueDate: today.toIso());
    final task = created.when(ok: (t) => t, err: (_) => throw StateError('expected Ok'));
  ...
  test('deleteTask soft-deletes: it disappears from every view', () async {
    final today = CivilDate.fromDateTime(DateTime.now());
    final created = await repository.createTask(userId: 'u1', title: 'Gone soon', dueDate: today.toIso());
    final task = created.when(ok: (t) => t, err: (_) => throw StateError('expected Ok'));
  ...
  test('subtasks: add, complete, and watch reflect state', () async {
    final created = await repository.createTask(userId: 'u1', title: 'Plan the trip');
    final task = created.when(ok: (t) => t, err: (_) => throw StateError('expected Ok'));
    ...
    expect(updated.firstWhere((s) => s.id == subtasks.first.id).isCompleted, isTrue);
  });
}
